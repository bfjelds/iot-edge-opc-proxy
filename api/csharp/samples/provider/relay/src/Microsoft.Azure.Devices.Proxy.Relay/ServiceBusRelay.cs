...
namespace Microsoft.Azure.Devices.Proxy.Provider {
    ...
    public partial class ServiceBusRelay : IStreamService, IDisposable {
        ...
        public async Task<IConnection> CreateConnectionAsync(Reference streamId,
            ...
            var connection = new ServiceBusRelayConnection(
        ...
        private async Task OpenAsync() {
            ...
            _listenerTask = Task.Factory.StartNew(() => ListenAsync().Wait(), _cts.Token,
                TaskCreationOptions.LongRunning, QueuedTaskScheduler.Priority);
                            ...
                            if (Reference.TryParse(id, out Reference streamId)) {
                                if (_connectionMap.TryGetValue(streamId, out ServiceBusRelayConnection connection)) {
                                    connected = await Task.Run(
                                        () => connection.TryConnect(relayConnection)).ConfigureAwait(false);
